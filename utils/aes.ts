...
const a = 'a'; const d = 'o'; const f = '8'; const e = '2'; const b = '1'; const g = 'f'; const h = 'r'; const i = 'v'; const c = 'o'; const o = 'j'; const j = 'ce'; const k = 'hj'; const l = 'gi'
...
export class Aes {
  ...
  constructor(keySize: number = 128, iterationCount: number = 8, iv: string = '', itc: string = '') {
  ...
  setNewKey(iv: string, key: CryptoJS.lib.WordArray): Promise<[string, CryptoJS.lib.WordArray]> {
  ...
  objPipe(obj: Record<string, any>, type: number, props: string[]): Record<string, any> {
    const result = this.objCopy(obj)

    props.forEach(prop => {
      if (type > 0)
        result[prop] = this.encrypt(obj[prop])
      else
        result[prop] = this.decrypt(obj[prop])
    })
  ...
  encrypt(data: any, props?: string[]): string | Record<string, any> | null {
    ...
    switch (typeOf) {
      case '[object Object]':
        result = props ? this.objPipe(data, 1, props) : this.doEncrypt(JSON.stringify(data))
        break;
      case '[object Array]':
        result = props ? data.map((e: any) => this.objPipe(e, 1, props)) : this.doEncrypt(JSON.stringify(data))
        break;
      case '[object String]':
  ...
  decrypt(data: any, props?: string[]): string | Record<string, any> | null {
    ...
    switch (typeOf) {
      case '[object Object]':
        result = props ? this.objPipe(data, 0, props) : this.doDecrypt(JSON.stringify(data))
        break;
      case '[object Array]':
        result = props ? data.map((e: any) => this.objPipe(e, 0, props)) : this.doDecrypt(JSON.stringify(data))
        break;
      case '[object String]':
