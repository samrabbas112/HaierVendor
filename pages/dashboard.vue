...
const secondGraphLabel =
  authStore.user?.user_type === "vendor" ? "Customers" : "Vendors";
const viewAllPath =
...
const getChartData = async () => {
  try {
    ...
    console.log(
      "*****************common/dashboard/orders/graphs res***********",
      res,
    );
  } catch (err) {
...
const getCustomerData = async () => {
  try {
    ...
    let url =
      authStore.user?.user_type == "vendors"
        ? "common/dashboard/vendor/graphs"
        : "common/dashboard/customer/graphs";
...
const getOrderSummery = async () => {
  try {
    ...
    const res = await api.makeRequest(
      "common/dashboard/orders/summary",
      "post",
      params,
    );
    ...
    if (res?.data) {
      ...
      Object.keys(res?.data).forEach((key) => {
        ...
        const label =
          `${replaceUnderscoreWithSpace(key)} ${res?.data?.[key]}` || 0;
...
const handleDateChange = async (newDate, isCalendarOpen, key) => {
  ...
  const wasPreviousDateNull =
    !lastDateRanges.has(key) || !lastDateRanges.get(key);

  if (isNewDateNull && wasPreviousDateNull) {
    console.log(
      "No change detected and both dates are null, skipping API call.",
    );
...
const exportData = async () => {
  try {
    ...
    const filter =
      intervalType.value === "custom" ? "custom" : intervalType.value;
        ...
        <VCardItem title="Dashboard">
          <template #append>
            <div class="d-flex align-items-center gap-5">
              <div>
                <VBtn color="info" @click="exportData">
                  Export
                  <VIcon end icon="tabler-arrow-big-up-lines" />
                </VBtn>
              </div>
              <div class="date-picker-wrapper" style="width: 315px">
                <AppDateTimePicker
                  v-model="rangeDate"
