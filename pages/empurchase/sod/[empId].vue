...
let products = {productCoverImg: '', productName: '', purchaseNum: 0, specName: ''};
  ...
  try {
    ...
    if(response && response.success) {
      ...
      orderDetail.value = order.value.orderProduct.map(orderProduct => {
        ...
        isDisabled.value = order.value.sod_status != 0;
        if(isDisabled.value){
          selectedRadio.value = order.value.finance_status;
          financeCode.value = order.value.dn_number;
...
const updateOrder =  async () => {
  ...
  try {
    const data = {
      'sod_status': selectedRadio.value,
      'rejection_reason': rejectReason.value,
      'dn_number': financeCode.value
    };
    console.log('wasim data',data);
    const response = await apiRequestObj.makeRequest(
      `haier/employee-order/sod-update/${route.params.empId}`,
      'post',
      data,
    )
          ...
          <VCardText class="d-flex flex-column gap-y-6">
            ...
            >
                  <VRadioGroup v-model="selectedRadio" inline>
                    <VRadio label="Success" color="success" value="1"
                            :disabled="isDisabled"
                    />
                    <VRadio label="Reject" color="error" value="2"
                            :disabled="isDisabled"
                    />
                  </VRadioGroup>
              ...
              <VCol
                v-if="selectedRadio != 0 "
                cols="12"
                sm="12">
                <VBtn class="me-3 w-100" @click="updateOrder">
